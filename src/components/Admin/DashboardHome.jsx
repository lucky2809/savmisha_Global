...
function SocialBadge({ label, entry }) {
  ...
  return (
    <Badge tone="danger" className="max-w-full">
      <MdCancel className="h-3.5 w-3.5 shrink-0" />
      <span className="truncate" title={entry.error || "Failed"}>
        {label}
      </span>
    </Badge>
  );
        ...
        <Card className="mt-6 border-amber-200 bg-amber-50">
          <div className="px-5 py-4">
            ...
            <ul className="mt-2 space-y-1.5">
              {products.flatMap((p) =>
                ["facebook", "instagram"]
                  .filter((k) => p.socialStatus?.[k]?.status === "failed")
                  .map((k) => (
                    <li key={`${p._id}-${k}`} className="text-xs text-amber-800">
                      <span className="font-medium capitalize">{k}</span>
                      {" - "}
                      {p.socialStatus[k].error || "no reason recorded"}
                    </li>
                  ))
              )}
            </ul>
